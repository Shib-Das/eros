...
use crate::app::ProgressUpdate;
 use crate::video::{extract_frames, is_video};
...
/// Removes duplicate videos by comparing fingerprints of sampled frames.
 pub async fn remove_duplicate_videos(
    selected_dirs: &[PathBuf],
    tx: &mpsc::Sender<ProgressUpdate>,
) -> Result<()> {
    let mut video_files: Vec<PathBuf> = selected_dirs
        .iter()
        .flat_map(|dir| {
            WalkDir::new(dir)
 .into_iter()
 ...
 .filter(|e| {
 e.file_type().is_file()
 && is_video(e.path().to_str().unwrap_or("")).unwrap_or(false) //
 })
 .map(|e| e.path().to_path_buf())
 })
 .collect();
...
let mut video_fingerprints: HashMap<PathBuf, Vec<u64>> = HashMap::new();
 for path in &video_files {
 // This reuses your existing frame extraction logic.
 if let Ok(frames) = extract_frames(path) {
 if !frames.is_empty() {
 let fingerprints: Vec<u64> = frames.iter().map(calculate_fingerprint).collect();
 video_fingerprints.insert(path.clone(), fingerprints);
 }
 }
 }
...
let mut duplicates_to_remove = HashSet::new();
 ...
 let path1 = &video_files[i];
 let path2 = &video_files[j];

if duplicates_to_remove.contains(path1) || duplicates_to_remove.contains(path2) {
 continue;
 }

if let (Some(prints1), Some(prints2)) =
 (video_fingerprints.get(path1), video_fingerprints.get(path2))
 {
 // For simplicity, we only compare videos with the same number of extracted frames.
 if prints1.len() != prints2.len() || prints1.is_empty() {
 continue;
 }

let total_distance: u32 = prints1
 .iter()
 .zip(prints2.iter())
 .map(|(&h1, &h2)| hamming_distance(h1, h2))
 .sum();

let avg_distance = total_distance as f64 / prints1.len() as f64;

if avg_distance <= 6.0 {
 duplicates_to_remove.insert(path2.clone());
 let similarity = 100.0 * (1.0 - (avg_distance / 64.0));
 let message = format!(
 "Duplicate video found: {:?} is {:.1}% similar to {:?}. Removing.",
 path2.file_name().unwrap(),
 similarity,
 path1.file_name().unwrap()
 );
...
for file_path in duplicates_to_remove {
 if fs::remove_file(&file_path).is_ok() {
 let message = format!("Removed duplicate video: {:?}", file_path);
 tx.send(ProgressUpdate::Message(message)).await?;
 }
 }
